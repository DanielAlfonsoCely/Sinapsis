...
func (s *EntidadService) publishAudit(ctx context.Context, actorID uuid.UUID, op models.AuditOperation, tabla string, registroID *uuid.UUID, err error, im models.ImportanceLevel) {
	var detalles *string
	if err != nil {
		msg := err.Error()
		detalles = &msg
...
func (s *EntidadService) Create(ctx context.Context, actorID uuid.UUID, req models.CreateEntidadRequest) (models.Entidad, error) {
	...
	s.publishAudit(ctx, actorID, models.AuditCreate, "entidad", registroID, err, models.Informative)
	return entidad, err
}
...
func (s *EntidadService) ListAdmin(ctx context.Context, actorID uuid.UUID, q string) ([]models.AdminEntidadListItem, error) {
	items, err := s.repo.ListAdmin(ctx, q)
	s.publishAudit(ctx, actorID, models.AuditConsult, "entidad", nil, err, models.Informative)
	return items, err
}
...
func (s *EntidadService) GetByIDAdmin(ctx context.Context, actorID, id uuid.UUID) (models.EntidadDetalle, error) {
	detalle, err := s.repo.GetByIDAdmin(ctx, id)
	s.publishAudit(ctx, actorID, models.AuditConsult, "entidad", &id, err, models.Informative)
	return detalle, err
}

// Stats devuelve métricas agregadas de la plataforma para el dashboard admin.
func (s *EntidadService) Stats(ctx context.Context, actorID uuid.UUID) (repositories.PlatformStats, error) {
