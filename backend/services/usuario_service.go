...
func (s *UsuarioService) publishAudit(ctx context.Context, actorID uuid.UUID, op models.AuditOperation, targetID uuid.UUID, err error, im models.ImportanceLevel) {
	var detalles *string
	if err != nil {
		msg := err.Error()
		detalles = &msg
...
func (s *UsuarioService) Create(ctx context.Context, actorID uuid.UUID, req models.CreateUsuarioAdminRequest) (models.Usuario, error) {
	...
	s.publishAudit(ctx, actorID, models.AuditCreate, user.ID, err, models.Informative)
	return user, err
}
...
func (s *UsuarioService) Delete(ctx context.Context, targetID, actorID uuid.UUID) error {
	if targetID == actorID {
		...
		s.publishAudit(ctx, actorID, models.AuditDelete, targetID, err, models.Critical)
		return err
	}
...
func (s *UsuarioService) AssignRole(ctx context.Context, targetID, actorID uuid.UUID, role string) error {
	...
	s.publishAudit(ctx, actorID, models.AuditChangePermission, targetID, err, models.High)
	return err
}
